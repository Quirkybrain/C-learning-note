...
/**
 * @brief AnimalVtbl::destroy 的 Cat 实现。
 *
 * 抽象层只传进来 Animal**，因此这里先通过 container_of() 找回完整
 * Cat 对象，再释放真正由 malloc() 返回的地址。释放后把 *self 置为
 * NULL，避免调用方继续持有已经失效的 Animal*。
 *
 * @param self 指向 Cat 对象内嵌 Animal 基类指针的地址。
 */
...
/**
 * @brief 将抽象层定义的 speak/drink/destroy 操作绑定到 Cat 的具体实现。
 */
