...
3. 判断输入来源
   1. 文件：通过 `lua_dofile()` 进入，再由 `lua_openfile()` 打开输入源
   2. 交互形式：逐行读取输入，再通过 `lua_dostring()` 进入，由 `lua_openstring()` 打开字符串输入源
4. lex yylex 进行词法分析
5. `lua_parse()` 调用 `yyparse()` 进行语法分析
6. 语法分析阶段直接生成字节码，并通过 `lua_execute()` 进行执行
7. 创建数据
   1. `table.c`：符号表、常量表、字符串表
   2. `hash.c`：lua 中 `table` 数据类型的底层实现
8. 在字符串和数组创建达到阈值时触发 GC，对虚拟栈以及全局符号表进行标记，并回收字符串和数组对象
...
巧妙通过 union struct 的组合将 C 语言中不同的类型 (`float`、`char*` 以及其它指针类型) 进行了封装，对 lua 仅暴露了 `Object` 的简单类型接口。lua 解释器不需要再担心不同类型的区分与存储困难，只需要通过 `tag` 进行区分放在栈中的是什么 lua 数值。这体现了设计中的封装思想。
...
- table 进行索引操作需要被索引的对象是 `T_ARRAY` 类型
...
lua 中很多栈都是采用了 C 中的数组，函数调用时又需要在同一段虚拟栈中区分“函数对象、调用标记、参数、返回值”等区域。为了解决这些问题，引入了不对用户可见的 `T_MARK` 类型。它在代码中的主要作用是：
...
`src/table.c` 中设计了三个关键的表用于辅助 lua 的正常运行。
...
1. 词法分析阶段识别到字符串字面量后，会直接把它放入常量表，并返回一个索引
2. 语法分析阶段处理诸如记录字段名之类的名字时，也会调用 `lua_findconstant()` 将其放入常量表

后续字节码中并不直接嵌入完整字符串，而是通过 `PUSHSTRING + 常量索引` 的形式引用它。
...
GC 的真正触发点不在 `lua_pack()` 本身，而是在创建字符串和数组对象时检查阈值：

- `lua_createstring()` 在字符串表接近上限或实体计数达到阈值时触发 GC
- `lua_createarray()` 在实体计数达到阈值时触发 GC

`lua_pack()` 负责执行一次完整的标记与清扫，其流程非常清晰：
...
对于 `Node` 的设计源自于 Table 结构本质上是一个映射：
